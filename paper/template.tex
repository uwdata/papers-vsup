...
\abstract{
	...
	Uncertainty is a vital part of many datasets, and should be conveyed accurately to the analyst.
	Directly incorporating uncertainty requires that designers construct bivariate mappings that encode value and uncertainty simultaneously.
	In this paper, we present a technique for generating such bivariate maps for uncertainty, ``Value-suppressing uncertainty maps'' (VSUMs).
	As opposed to traditional bivariate maps, VSUMs make the informed choice to alias together values when uncertainty is high, in order to afford greater discriminability between categories when uncertainty is low. That is, certain data is intrinsically easier to visually disambiguate than uncertain data.
	Values with low uncertainty can be more precisely compared, whereas similar values with higher uncertainty will be harder to differentiate in accordance with their variability.
	By making informed decisions about where to create category bins, VSUMs make better use of the limited budget of discriminable marks in bivariate maps, and dissuade analysts from making decisions based on data with sufficiently high uncertainty.
	We present several examples of VSUMs, as well as a crowd-sourced evaluation showing that, compared to more traditional bivariate maps, VSUMs promote decision-making that places greater weight on uncertainty information compared to traditional bivariate maps.
