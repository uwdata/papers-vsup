...
\newcommand{\flowFig}{
	\begin{figure}[t]
		...
		In this example two colors from the map for $n=9$ are too close in CIELAB space. Thus, we create a VSUM with $n=8$ bins that encodes value with Viridis and uncertainty with increasingly white colors.}
		\label{fig:flow}
	\end{figure}
}
...
\begin{figure}[t]
	...
	\vspace{-15px}
	\caption{Variability for 25 different viral populations of the SIV virus, encoded with a traditional 2D bivariate map. Error in aligning short reads can lead to poor quality data, and so the amount of these errors is encoded as the uncertainty. }
	\label{fig:viral2d}
...
\newcommand{\responseTimeFig}{
	\begin{figure}[t]
		\centering
		\includegraphics[width=.8\columnwidth]{rt1.png}
		\caption{The effect of chart type on response time for the identification task. Juxtaposed maps of value and uncertainty introduce a visual search task, increasing the time to complete even simple tasks involving both value and uncertainty information. The confidence intervals are bootstrapped 95\% CIs.}
		\label{fig:rt1}
	\end{figure}
}
...
\newcommand{\accuracyFig}{
	\begin{figure}[t]
		\centering
		\includegraphics[width=.8\columnwidth]{accuracy1.png}
...
\newcommand{\uncertaintyFig}{
	\begin{figure}[t]
		\centering
		\includegraphics[width=.8\columnwidth]{uncertainty2.png}
