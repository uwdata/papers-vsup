...
\newcommand{\taskTwoFig}{
	\begin{figure}
		...
		\caption{The prediction task. The participant has a list of locations, and ought to place their ships on locations with low probability of attack, and high certainty in this probability.}
...
\newcommand{\taskTwoQuartiles}{
	\begin{figure}
		\centering
		\includegraphics[width=0.75\columnwidth]{exp2histogram.pdf}
		\caption{2D histogram of number of guesses from the prediction experiment for the cases when participants saw a traditional bivariate maps (left) and VSUPs (right). For each trial, participants made 5 guesses. While participants generally favored low uncertainty in guesses, our stimuli design encouraged them to make at least some guesses with higher uncertainty. VSUPs encouraged more guesses with middling uncertainty, and discouraged highly uncertain guesses.}
		\label{fig:taskTwoQuartiles}
	\end{figure}
}
...
\newcommand{\pollFig}{
	\begin{figure}[t]
		...
		\vspace{-15px}
		\caption{Polling data prior to the 2016 U.S. Presidential Election, encoded in a traditional 2D bivariate map. Many states appear to be ``leaning'' towards Clinton, even when the margin of error of the polls is quite large. The blue-red color scale that is semantically appropriate for the topic is also not isoluminant, making it difficult to see just how much uncertainty there is in states with small leads for either candidate.}
		\label{fig:polling2d}
		...
		\vspace{-15px}
		\caption{Polling data prior to the 2016 U.S. Presidential Election, encoded with a VSUP. The larger amount of color bins for high-certainty regions shows that the leads for Hillary Clinton were, in many states, quite small. The collapsing of bins as uncertainty increases dissuades viewers from making confident predictions about states where the outcome was very much in doubt. }
