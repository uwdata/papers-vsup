...
The experiment was a within-subjects design with 1 factor (encoding type) with $3$ levels. \figref{fig:conditions} shows examples of these factor levels. We had 8 replications for each level for the identification task, and 6 for the prediction task, for a total of $24$ stimuli for the first task, and $18$ for the second. We discuss the two experimental tasks in more detail in the following sections. Experimental materials, including data tables and stimuli generation code, are available at \url{URL-REMOVED-FOR-REVIEW}.
